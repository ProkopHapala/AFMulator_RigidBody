...
void fileManager::importDataFromFiles( const char functionName[500], quaterDataManager* dataArray ){   // import quaternion data from multiple files
	for( int i = 0; i < numOfFiles; i++ ){
		listOfFiles[i]->importDataFromFile( functionName, (*dataArray)[i] );
	}
}

void fileManager::importDataFromFiles( const char functionName[500], vectorDataManager* dataArray ){   // import vector data from multiple files
	for( int i = 0; i < numOfFiles; i++ ){
		listOfFiles[i]->importDataFromFile( functionName, (*dataArray)[i] );
	}
}

void fileManager::exportDataToFiles( const char functionName[500], vectorDataManager* dataArray ){    // export vector data to multiple files
	for( int i = 0; i < numOfFiles; i++ ){
		listOfFiles[i]->exportDataToFile( functionName, (*dataArray)[i] );
	}
}

void fileManager::exportDataToFiles( const char functionName[500], quaterDataManager* dataArray ){    // export quaternion data to multiple files
	for( int i = 0; i < numOfFiles; i++ ){
		listOfFiles[i]->exportDataToFile( functionName, (*dataArray)[i] );
	}
}

fileWrapper* fileManager::getFile( int i ){  // get i-th file
	if( i > numOfFiles ){
...
fileWrapper* fileManager::operator [] ( int i ){  // get i-th file
	if( i > numOfFiles ){
...
fileManager::fileManager( int n )   // constructor
: numOfFiles( n ){
...
fileManager::fileManager( int n, std::string fileNamePrefix_ )  // constructor
...
fileManager::fileManager( int n, const char* fileNamePrefix_ )  // constructor
: numOfFiles( n ), fileNamePrefix( fileNamePrefix_ ){
	listOfFiles = new fileWrapper*[numOfFiles];
	char str[400];
	for( int i = 0; i < numOfFiles; i++ ){
...
fileManager::fileManager( int n, std::string* listOfFileNames, int* listOfMoleculeInstances )   // constructor
: numOfFiles( n ){
	CHECK(numOfFiles)
