...
//	void initRenderer();
	...
	void updateScreen( PhysicalSystem* world );
	void drawScreen( PhysicalSystem* world );
	...
	void oneActionBack( PhysicalSystem* world );	
	...
	bool mousePickAtom( PhysicalSystem* world );
	bool mousePickAtom( PhysicalSystem* world, bool emphasized );
	bool mouseMoveAtom( PhysicalSystem* world );
	bool mouseMoveMolecule( PhysicalSystem* world );
	void mouseSetAuxPoint( PhysicalSystem* world );
	...
	void addPickedAtom( PhysicalSystem* world );
	bool selectionChosen( PhysicalSystem* world );
	bool destroySelection( PhysicalSystem* world );
	
	// event handling
	
	void inputHandlingBare( SDL_Event event, PhysicalSystem* world, bool & loopEnd );
	
	void handleKeyDown( SDL_Event event, PhysicalSystem* world );
	void handleMouseMotion( SDL_Event event, PhysicalSystem* world );
	void handleMouseButtonDown( SDL_Event event, PhysicalSystem* world );
	void handleMouseButtonUp( SDL_Event event, PhysicalSystem* world );
	void handleMouseAndKeyStates();
	void handleMouseWheel( SDL_Event event, PhysicalSystem* world );
};
...
void Screen::updateScreen( PhysicalSystem* world ){
// update the screen scene
	...
	// store the camera position in appropriate variables
	getCameraDirections();
...
//void Screen::drawScreen( PhysicalSystem* world ){
//// draw the screen
...
void Screen::setAxisView( AXES_VIEW axis ){
...
// store position of a camera w.r.t. the screen in camRight, camUp and camDir
...
// set a viewpoint of a camera
	...
	// rotate the camera according to QCamera
...
bool Screen::mousePickAtom( PhysicalSystem* world ){
// pick an atom providing the mouse clicks on it

	Vec3d mouseCoords, a, b, c;
	float mouseA, mouseB;
	
	// rescale mouse positions on a screen
	rescaleMousePositions( mouseA, mouseB );
	
	// set to mouseCoords the position of a mouse in the system coordinates (without the z-component)
	mouseCoords.set_lincomb( mouseA, camRight, mouseB, camUp );
	
	// mouse picking itself
	bool found = world->rayPickAtomSet( mouseCoords, camDir, true );
	
	return found;
}

bool Screen::mousePickAtom( PhysicalSystem* world, bool emphasized ){
// pick an atom providing the mouse clicks on it
	...
	// mouse picking itself
	bool found = world->rayPickAtomSet( mouseCoords, camDir, emphasized );
	
	return found;
}

bool Screen::mouseMoveAtom( PhysicalSystem* world ){
// move a picked atom where the mouse is
	...
	// moving itself
	bool found = world->moveAtom( atomSystemCoords );
//	bool found = false;
...
bool Screen::mouseMoveMolecule( PhysicalSystem* world ){
...
void Screen::mouseSetAuxPoint( PhysicalSystem* world ){
// move a picked molecule where the mouse is
	...
	// set auxiliary point
	world->mouseSetAuxPoint( mouseA, mouseB, camRight, camUp, camDir );

}

// ============== SELECTION OF ATOMS ==============

bool Screen::selectionChosen( PhysicalSystem* world ){
// is the selection of atoms picked by a mouse?

	return world->selectionChosen();

}

bool Screen::destroySelection( PhysicalSystem* world ){
// destroy the selection of atoms

	return world->destroySelection();

}

void Screen::addPickedAtom( PhysicalSystem* world ){
// add a picked atom
...
void Screen::initializeContextMenu(){
// initilize a context menu at the beginning of the application
...
void Screen::createContextMenuFromSetting( contextMenuType type ){
// show a context menu on the screen
...
void Screen::destroyContextMenu(){
// destroy a context menu

	contMenu->destroyMenu();
	
}

void Screen::rescaleContextMenu( float rescaleFactor ){
// adjust context menu width and height so it is not effectively affected by zooming
...
void Screen::inputHandlingBare( SDL_Event event, PhysicalSystem* world, bool & loopEnd ){
// handle the input events
	...
	switch( event.type ){
		...
		case SDL_MOUSEBUTTONUP:
			handleMouseButtonUp( event, world );
			break;
			
		case SDL_MOUSEWHEEL:
			handleMouseWheel( event, world );
			break;
	}
...
void Screen::handleKeyDown( SDL_Event event, PhysicalSystem* world ){
// handle an event when some key is pressed down
...
void Screen::handleMouseMotion( SDL_Event event, PhysicalSystem* world ){
// handle an event when the mouse moves
...
void Screen::handleMouseButtonUp( SDL_Event event, PhysicalSystem* world ){
// handle an event when the mouse button is released
...
void Screen::handleMouseButtonDown( SDL_Event event, PhysicalSystem* world ){
// handle an event when the mouse button is pressed down
		...
		if( contMenuExists ){
			...
			if( contMenu->isActivated( mouseA, mouseB ) ){
				...
				switch( contMenu->getAction() ){
					...
					case contextMenu::ACTION_EXPORT:
						world->exportData();
						break;
					case contextMenu::ACTION_ADJUST:
						world->setFromStdinAdjustMolecule();
						break;
					case contextMenu::ACTION_ADJUST3:
						world->setFromStdinAdjustMoleculeSelection();
						break;
					case contextMenu::ACTION_ADJUST_X:
						world->adjustMoleculeSelectionXaxis();
						break;
					case contextMenu::ACTION_ADJUST_Y:
						world->adjustMoleculeSelectionYaxis();
						break;
					case contextMenu::ACTION_ADJUST_Z:
						world->adjustMoleculeSelectionZaxis();
						break;
					case contextMenu::ACTION_DIAL:
//						contMenu->redrawTexture( button::BUTTON_DIAL, floatToString( distance ) );
						break;
...
void Screen::handleMouseAndKeyStates(){
// handle an event when some keys are hold pressed
...
void Screen::handleMouseWheel( SDL_Event event, PhysicalSystem* world ){
// handle an event when a mouse wheel is rotated
...
//void Screen::replayMoveLeft(){
//// in a replay mode move the scanning tip one step to the left

//}

//void Screen::replayMoveRight(){
//// in a replay mode move the scanning tip one step to the right

//}

//void Screen::replayMoveDown(){
//// in a replay mode move the scanning tip one step down

//}

//void Screen::replayMoveUp(){
//// in a replay mode move the scanning tip one step up
