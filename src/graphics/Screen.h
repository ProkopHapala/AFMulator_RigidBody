...
void Screen::mouseSetAuxPoint( PhysicalSystem* world ){   // move a picked molecule where the mouse is
	float mouseA, mouseB, z_coord;
...
bool Screen::selectionChosen( PhysicalSystem* world ){   // is the selection of atoms picked by a mouse?
	return world->selectionChosen();
}

bool Screen::destroySelection( PhysicalSystem* world ){  // destroy the selection of atoms
	return world->destroySelection();
}

void Screen::addPickedAtom( PhysicalSystem* world ){  // add a picked atom
	char str[40];
...
void Screen::inputHandlingBare( SDL_Event event, PhysicalSystem* world, bool & loopEnd ){  // handle the input events
	switch( event.type ){
...
void Screen::handleKeyDown( SDL_Event event, PhysicalSystem* world ){  // handle an event when some key is pressed down
	destroyContextMenu();
...
void Screen::handleMouseMotion( SDL_Event event, PhysicalSystem* world ){   // handle an event when the mouse moves
...
void Screen::handleMouseButtonUp( SDL_Event event, PhysicalSystem* world ){  // handle an event when the mouse button is released
...
void Screen::handleMouseButtonDown( SDL_Event event, PhysicalSystem* world ){  // handle an event when the mouse button is pressed down
	setAxisView( AXIS_NONSPEC );
	float mouseA, mouseB;
	rescaleMousePositions( mouseA, mouseB );
	// left mouse button
...
void Screen::handleMouseWheel( SDL_Event event, PhysicalSystem* world ){ // handle an event when a mouse wheel is rotated
	destroyContextMenu();
	if( event.wheel.y > 0 ){
		zoom /= VIEW_ZOOM_STEP;
