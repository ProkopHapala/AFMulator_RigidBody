...
class tipOnePoint : public abstractTip {
// tip represented by a fixed point
...
class tipOnePointMol : public tipOnePoint {
// tip with a probe molecule
...
tipOnePoint::tipOnePoint()
// constructor
: abstractTip()
{
}

tipOnePoint::tipOnePoint( Vec3d xaxis, Vec3d yaxis, Vec3d zaxis )
// constructor
: xaxis( xaxis ), yaxis( yaxis ), zaxis( zaxis )
{
}

tipOnePoint::tipOnePoint( Vec3d zaxis )
// constructor
: zaxis( zaxis )
{
}

void tipOnePoint::printTip(){
// print parameters
...
tipOnePointMol::tipOnePointMol( char* filename )
// constructor
: tipOnePoint()
{
...
void tipOnePointMol::setTipParameters( char* filename ){
// sets parameters of the tip
	...
	if( tipfile ){
		...
		sscanf( str, "%lf", &kcoeff );

		readLineComment( tipfile, str );
		sscanf( str, "%lf", &kcoeffrot );

		readLineComment( tipfile, str );
		sscanf( str, "%lf", &kcoeffrot2 );

		readLineComment( tipfile, str );
		sscanf( str, "%lf", &kcoeffrot3 );

		readLineComment( tipfile, str );
		sscanf( str, "%lf %lf %lf", &tipOnePoint::yaxis.x, &tipOnePoint::yaxis.y, &tipOnePoint::yaxis.z );
		yaxis.normalize();

		readLineComment( tipfile, str );
		sscanf( str, "%lf %lf %lf %lf", &rotInitAux.x, &rotInitAux.y, &rotInitAux.z, &rotInitAux.w );
...
void tipOnePointMol::setTipParams( const char* filename	){
// set parameters pertaining to tip geometry
	...
	if( tipFile ){
		...
		readLineComment( tipFile, str );
		count = sscanf( str, "%i", &probeMol->atom );

		readLineComment( tipFile, str );
		count = sscanf( str, "%lf %lf %lf", &probeMol->pos.x, &probeMol->pos.y, &probeMol->pos.z );

		readLineComment( tipFile, str );
		count = sscanf( str, "%lf %lf %lf %lf", &probeMol->rot.x, &probeMol->rot.y, &probeMol->rot.z, &probeMol->rot.w );

		readLineComment( tipFile, str );
		count = sscanf( str, "%399s", tipSettingFile );
